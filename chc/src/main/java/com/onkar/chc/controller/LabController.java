...
public class LabController {
    ...
    // Pathologist uploads a lab report with file (requires existing test request ID)
    ...
    // Standalone upload: Pathologist creates request AND uploads report in one step
    @PostMapping("/uploadStandaloneReport")
    public ResponseEntity<String> uploadStandaloneReport(
            @RequestParam("patientHealthCardId") String patientHealthCardId,
        ...
        log.info("Standalone lab report upload by Pathologist: {} for patient: {}", pathologistUserName, patientHealthCardId);

        // Step 1: Auto-create a lab test request
        LabTestRequestEntity testRequest = LabTestRequestEntity.builder()
                .doctorUserName(pathologistUserName)
                .patientHealthCardId(patientHealthCardId)
                .testName(testName)
                .notes(remarks != null ? remarks : "")
                .status("PENDING")
                .build();
        testRequest = labTestRequestRepo.save(testRequest);

        // Step 2: Store file
        String fileName = fileStorageService.storeFile(file);

        // Step 3: Create lab report
        LabReportEntity reportEntity = LabReportEntity.builder()
                .labTestRequest(testRequest)
                .pathologistUserName(pathologistUserName)
                .findings(findings)
